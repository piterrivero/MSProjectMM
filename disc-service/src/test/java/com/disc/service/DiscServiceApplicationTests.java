...
class DiscServiceApplicationTests {
	...
	public void shouldListAllDiscs() {
		...
		assertThat(discList.size()).isEqualTo(3);
	}
	...
	public void shouldListAllLiveDiscs() {
		...
		assertThat(discList).isNotNull();
		assertThat(discList).isNotEmpty();
		assertThat(discList.size()).isEqualTo(1);
	}
	...
	public void shouldGetDiscByIdBand() {
		// GIVEN
		List<Disc> discListBandMock = discListMock.stream().filter(x -> x.getIdBand() == 1).collect(Collectors.toList());
